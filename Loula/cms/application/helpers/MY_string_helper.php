...
{
	$CI = &get_instance();
	$CI->load->helper('variable');
	// merge params
	$params = array_merge(array('end' => '', 'end_sentence' => true, 'strip_tags' => TRUE),$params);
	...
	elseif(isset($params['end_sentence']) && $params['end_sentence'] == true)
	{
		// end last sentence ending
		$chars = array('.','?','!');
		foreach($chars as $char)
		{
			$punctuation[] = strrpos($text, $char)+1;
		}
		$text = substr($text, 0, max($punctuation));
	}
	// if tags are not striped, close open tags
	if( boolean($params['strip_tags']) == false )
	{
		closetags($text);
	}
	// return $text
