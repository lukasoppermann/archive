...
class media_model extends MY_Model {
	...
	function get_home_images()
	{
		...
		foreach($media as $key => $m)
		{
			...
			//
			if($m['instore'] != false && $m['instore'] != 'false')
			{
				$output['instore'] = $m;
			}
			elseif($m['eboutique'] != false && $m['eboutique'] != 'false')
			{
				$output['eboutique'] = $m;
			}
			elseif( $m['key'] == 'hero' )
			...
			foreach( $entries as $entry )
			{
				...
				if( is_array($key) && count($key) > 0 )
				{
					if( ( isset($entry['store']) && count($entry['store']) > 0 ) || isset($entry['sales_start']) )
					{
						...
						$media[$key[key($key)]]['label'] = '<span class="product-designer">'.'</span>'.$entry['title']; //$entry['title']
