...
(function($) {
	...
	$.widget("ui.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		myToArray: function(o) {

			o = o || {};
			var sDepth = o.startDepthCount || 0;
			var ret = [];

			// ret.push({"id":'root',"parent_id": 'root', "menu": $(this.element).attr("id")});

			$(this.element).children('li').each(function() {
				_recursiveArray($(this), sDepth + 1);
			});
			...
			function _recursiveArray(item, depth) {
				...
				if ($(item).children(o.listType).children('li').length > 0) {
					...
					$(item).children(o.listType).children('li').each(function() {
						_recursiveArray($(this), depth);
					});
					depth --;
				}
				id = $(item).children('div').attr('id').match(o.expression || (/(.+)[-=_](.+)/));
				if (depth === sDepth + 1) pid = '0';
				else {
					parentItem = $(item).parent(o.listType).parent('li').children('div').attr('id').match(o.expression || (/(.+)[-=_](.+)/));
					
					pid = parentItem[2];
				}
				ret.push({"id": id[2], "parent_id": pid});
