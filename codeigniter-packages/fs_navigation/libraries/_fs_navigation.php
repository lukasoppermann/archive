...
class CI_FS_navigation {
	...
	function path( $params = array(null) )
	{
		...
		{
			// merge params
			$params = array_merge($this->data, $this->params, $params);
			// -----------------
			// define seperator	
			$seperator		= !empty($this->data['path_seperator']) ? "<span class='separator'>".$this->data['path_seperator']."</span>" : '';
			// add path_before to path
			$path		 	= !empty($params['path_before']) ? $params['path_before'].$seperator : null;				
			// -----------------
			...
			foreach($this->active['items'] as $key => $item)
			{
				if($item['path'] == $this->current['path'])
				{
					...
					$path .= "<span class='current ".$params['path_item']."'>".$item['label']."</span>\n";
				}
				else
				{
					// show other items as link
					$path .= "<a class='".$params['path_item']."' href=\"".active_url(FALSE).$item['path']."\">".$item['label']."</a>\n".$seperator;
				}
			}
			// -----------------
			// return path
			return "\n<div".(!empty($params['path_id']) ? ' id="'.$params['path_id'].'"' : '').
					(!empty($params['path_class']) ? ' class="'.$params['path_class'].'"' : '').">\n".$path."</div>\n";
		}
		// if not item active return false
		return false;
	}
	// --------------------------------------------------------------------
	/**
	 ...
	 */
	function sitemap( $menus = array(NULL), $params = array() )
	{
		// merge params
		...
		// loop through menus
		foreach( $menus as $title => $id )
		{
			// get title
			$title = !is_string($title) || strlen($title) < 2 ? NULL : "<h4>".$title."</h4>";
			// get menu
			$menu[] = "<div class='menu_".$id."'>".$title.$this->tree(array('menu' => $id))."</div>\n";			
		}
		// return sitemap
		return  '<div'.(!empty($params['id']) ? ' id="'.$params['id'].'"' : '').
				(!empty($params['class']) ? ' class="'.$params['class'].'"' : '').'>'.implode('',$menu).'</div>';
	}
	// --------------------------------------------------------------------
	/**
	 * active
