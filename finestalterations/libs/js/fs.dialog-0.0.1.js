...
;(function( $, window, document )
{
	...
	var methods = {
		...
		fx: {
			slide: function()
			{
				methods.position(false);
				var top = ($(window).height()/2)-(_this.height()/2);
				_this.css({'marginTop':0,'top':-_this.height()+50}).show().animate({'top':top,'marginTop':'+=10'}, methods.settings.speed,
		...
		{ 
			...
			if( _dialog_content.length == 0 )
			{
				...
				if( _inner_wrapper.length == 0 || _dialog_wrapper.length == 0 )
				{
					...
					_dialog_wrapper = $('<div '+methods.get_selector(methods.settings.wrapper)+'>').html(_inner_wrapper.wrap('<div></div>').parents().html()).append('<div id="dialog_temp"></div>');
		...
		update: function(content, cache_name, fn)
		{
			// check for temp
			var _temp = _dialog_wrapper.find('#dialog_temp');
			if( _temp.size() > 0 )
			{
				_temp.html( content );
				var height 	= _temp.height();
				var width 	= _temp.width();
				if( _this.height() != height || _this.width() != width )
				{
					_this.animate({'height':height, 'width':width, 'marginTop':'-'+(height/2), 'marginLeft':'-'+(width/2)}, 200, function(){
						_dialog_content.hide().html( content ).fadeIn(300);
					});
				}
				else
				{
					_dialog_content.hide().html( content ).fadeIn(100)
				}
			}
			else
			{
	    ...
	    show: function( fn, args, cache_name, settings ) 
		{
			...
			if( _this == null )
			{
				...
				// add loading class
				if( (!methods.cache[cache_name] || methods.cache[cache_name] == null || args.refresh == true) && typeof args === 'object' && methods.fn[fn] )
				{
