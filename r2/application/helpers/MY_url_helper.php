...
function page_url($slash = TRUE)
{
	...
	if($slash == TRUE)
	{
		return $CI->config->slash_item('base_url').variable($CI->config->slash_item('url_parts')).$CI->config->unslash_item('lang_abbr').$CI->navigation->current('path').'/';
	}
	else
	{
		return $CI->config->slash_item('base_url').variable($CI->config->slash_item('url_parts')).$CI->config->unslash_item('lang_abbr').$CI->navigation->current('path');
...
function media()
{
	...
	{
		return $CI->config->slash_item('base_url').$CI->config->slash_item('dir_media').$args[0];	
	}
}
// ------------------------------------------------------------------------
/**
