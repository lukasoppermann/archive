...
class Homepage extends MY_Controller {
			...
			if( isset($blocks) && is_array($blocks) )
			{
				...
				foreach($blocks as $column => $b)
				{
					...
					foreach($b as $pos => $block)
					{
						...
						{
							db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $block['id']), array('data' => array('position' => $pos+1)), array('merge' => TRUE, 'json' => array('data')) );
						}
						// if image exists
	...
	function edit_block( )
	{
		...
		if( isset($data['id']) )
		{
				...
				foreach($columns as $c_id => $column)
				{
					if( !isset($done) || !in_array($c_id, $done) )
					{
						...
						foreach($column as $pos => $block)
						{
							...
							{
								db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $block['id']) , array('column' => $c_id, 'position' => $pos+1), array('merge' => TRUE, 'json' => array('data')) );
							}
						}
					}
				}
			...
			{
				// update db
				db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $data['id']), array($data['key'] => $data['value']), array('merge' => TRUE, 'json' => array('data')) );
	...
	function delete_image()
	{
		...
		if( $image['status'] == 4 )
		{
			unlink(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$image['filename'].'_'.config('homepage_thumb').'.'.$image['ext']);
		...
		if( isset($id) && $id != '' )
		{
			...
			{
				db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $id), array('data/image' => null), array('merge' => TRUE, 'json' => array('data')) );
			}
			else
			{
				db_delete(config('system/current/db_prefix').config('db_entries'), array('id' => $id));
	...
	function upload_image($id, $thumb)
	{
		...
		if( array_key_exists('file', $_FILES) && $_FILES['file']['error'] == 0 )
		{
			...
			// replace file
			move_uploaded_file($file['tmp_name'], config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$filename.'_'.$thumb.'.'.$file['ext']);
			// resize image if needed
			$img = getimagesize(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$filename.'_'.$thumb.'.'.$file['ext']);
			...
			{ 
				...
				$config['source_image'] = config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$filename.'_'.$thumb.'.'.$file['ext'];
				$config['quality'] = '100';
				// load config to lib
			...
			db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $id) , array('data' => array('image' => $image_id)), array('merge' => TRUE, 'json' => array('data')) );
			// send result
			echo json_encode(array('success' => 'true', 'id' => $image_id, 'dir' => config('display_image_dir/dir').'/'.$filename.'_'.$thumb.'.'.$file['ext']));
		}
		else
		{
