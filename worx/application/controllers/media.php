...
class Media extends MY_Controller {
	...
	function upload( $id = null )
	{
		...
		if( array_key_exists('pic', $_FILES) && $_FILES['pic']['error'] == 0 )
		{
			...
			while( file_exists(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$filename.'.'.$file['ext']) )
			{
				$filename = $file['filename'].'-'.rand(0,99);
			}
			$file['filename'] = $filename;
			//
			if(move_uploaded_file($file['tmp_name'], config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$file['filename'].'.'.$file['ext']))
			{
				$this->create_thumb($file['filename'], $file['ext'], config('system/current/dir').'/'.config('upload_image_dir/dir').'/');
				...
				if( $id != null )
				{
					...
					array('data/images' => array($image_id)), array('merge' => TRUE, 'json' => array('data')) );
				}
				// exit with message
	...
	function delete( $id = null )
	{
		...
		if( isset($image_id) && $image_id != null )
		{
			...
			// update db
			db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $id), $update, array('merge' => TRUE, 'json' => array('data')) );
		}
	}
	// --------------------------------------------------------------------
	/**
	 ...
	 */
	 function image_to_homepage()
	 {
		...
		if( isset($banners) && is_array($banners) )
		{
			...
			{
				if( isset($banner['data']) && $banner['data'] != '')
				{
		...
		// update data
		$updates[$new_id] = array('id' => $new_id, 'entry' => $this->input->post('entry'),'caption' => variable($this->input->post('value')), 'link' => $this->input->post('link'));
	 ...
	 */
	 function remove_image_homepage()
	 {
		...
		// delete
		db_delete(config('system/current/db_prefix').config('db_data'), array('type' => 'banner', 'id' => $update['id']));
		$update = $update['data'];
		// update data
		...
		if( isset($update) && is_array($update) && isset($update[key($update)]) )
		{
			// update config
			db_insert(config('system/current/db_prefix').config('db_data'), array('key' => 'settings', 'type' => 'banner', 'data' => json_encode($update)));
		}
	 }
	// --------------------------------------------------------------------
	/**
	...
	function edit_image()
	{
		...
		if( $old_filename['filename'] != $filename )
		{
			...
			// update db
			db_update(config('db_files'), array('id' => $this->input->post('id')), array('filename' => $filename), array('merge' => TRUE, 'json' => array('data')) );
			// dir
			$dir = config('system/current/dir').'/'.config('upload_image_dir/dir').'/';
	...
	function replace_image($id, $thumb)
	{
		...
		if( array_key_exists('file', $_FILES) && $_FILES['file']['error'] == 0 )
		{
			...
			$file = $_FILES['file'];
			// replace file
			move_uploaded_file($file['tmp_name'], config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$db_file['filename'].'_'.$thumb.'.'.$db_file['ext']);
			// resize image if needed
			$img = getimagesize(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$db_file['filename'].'_'.$thumb.'.'.$db_file['ext']);
			$size = config('thumbs/'.$thumb);
			// check if size is correct
			if(($img[0] != $size['width'] || $img[1] != $size['height']) && ( $thumb != 'lightbox' && $size['width'] != 'variable' && $size['height'] != 'variable'))
			{ 
				$img['width'] = $img[0];
				$img['height'] = $img[1];
				// if not, resize
				...
				$config['source_image'] = config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$db_file['filename'].'_'.$thumb.'.'.$db_file['ext'];
				$config['quality'] = '100';
				// load config to lib
			...
			// if one side is variable make it so that the other side fits
			elseif( $size['width'] == 'variable' xor $size['height'] == 'variable' )
			{
				// set height or width
				if( $size['height'] == 'variable' )
				{
					$config['width'] = $size['width'];
	...
	function create_thumb($file, $ext, $dir)
	{
		...
		foreach( config('thumbs') as $name => $sizes )
		{
			...
			{
				if( $sizes['width'] != "variable" && $sizes['height'] != "variable" )
				{
					// image is higher than wide
					if( $img_small == 'width' )
					{
					...
					$this->image_lib->resize();
					//
					$config['source_image'] = $dir.$config['new_image'];
					// set crop proportions
				...
				elseif( $sizes['width'] == "variable" )
				{
					$config['height'] = $sizes['height'];
					$config['width'] = $img['width'];
					$sizes['width'] = $config['width'];
					$config['master_dim'] = 'height';
					// init lib
					$this->image_lib->clear();
					$this->image_lib->initialize($config);
				...
				elseif( $sizes['height'] == "variable" )
				{
					$config['width'] = $sizes['width'];
					$config['height'] = $img['height'];
					$sizes['height'] = $config['height'];
					$config['master_dim'] = 'width';
					// init lib
					$this->image_lib->clear();
					$this->image_lib->initialize($config);
			...
			{
				copy($config['source_image'], $dir.$config['new_image']);
			}
		}
		return TRUE;
	}
	 ...
	 * module image
	 *
	 * @description	upload module image and add to entry
	 * 
	 */	
	function module_image( $post_id)
	{
		// if file is transmitted
		if( array_key_exists('file', $_FILES) && $_FILES['file']['error'] == 0 )
		{
			// delete old image if exists
			$old = db_select(config('system/current/db_prefix').config('db_entries'), array('id' => $post_id),array('single' => true));
			if( isset($old['module_image']) && $old['module_image'] != null )
			{
				$old_path = str_replace(config('display_image_dir/dir').'/','',$old['module_image']);
				unlink(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$old_path);
			}
			// get transfered file
			$file = $_FILES['file'];
			$name = $file['name'];
			// check if file exists
			if( file_exists(config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$file['name']) )
			{
				$ext = substr(strrchr($file['name'],'.'), 1); 
				$file_name = substr($file['name'], 0, strrpos($file['name'],'.'));
				$new_file = $file_name.'_'.rand(100,9999999).rand(100,9999999).'.'.$ext;
				$path = config('system/current/dir').'/'.config('upload_image_dir/dir').'/'.$new_file;
			}
			else
			{
				$new_file = $file['name'];
			...
			// update db
			db_update(config('system/current/db_prefix').config('db_entries'), array('id' => $post_id), array('data/module_image' => $new_file));
			// send result
			echo json_encode(array('success' => 'true', 'path' => config('display_image_dir/dir').'/'.$new_file));
		}
		else
		{
			echo json_encode(array('error' => 'File could not be uploaded.'));
