...
class MY_Config extends CI_Config {
	...
	function set_config_from_db($db = null)
	{
		...
		// merge params
		$keys = array('settings', 'system', 'user');
		...
		{	
			...
			if( isset($result['data']['_id']) )
			{
				...
				$config[$result['key']][$result['type']][$result['data']['_id']] = $result['data'];
			}
			else
			{
		...
		if( isset($config['settings']) )
		{
			foreach($config['settings'] as $type => $setting)
			{
				if(count($setting) == 1)
				{
					unset($setting[key($setting)]['id'], $setting[key($setting)]['key'], $setting[key($setting)]['type']);
					$config['settings'][$type] = $setting[key($setting)]['data'];
				}
			}
		}
		// -----------------------------------
	...
	function set_item($item, $value = null)
	{
		...
		else
		{
			...
			if(isset($this->config[$item]))
			{
				...
				//
				log_message('debug','Overwriting '.$item.' ('.$var.') with '.$value);
			}
		}
	}
// --------------------------------------------------------------------
