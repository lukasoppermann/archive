...
(function ($) {
  var DatePicker = function () {
    var ids = {},
       ...
       * calendar mode.
       */
      normalizeDate = function (mode, date, format) {
        ...
        if(date && (!$.isArray(date) || date.length > 0)) {
            ...
            if (mode != 'single') {
              if (!$.isArray(date)) {
				...
				{
					// if format is empty set default ['devider',day,month,year]
					if(!format) format = {'divider': '/','d' :0,'m' : 1,'y' : 2};
					var newDate = date.split(format['divider']);
					date = newDate[format['m']]+'/'+newDate[format['d']]+'/'+newDate[format['y']];
